...
const MySessionsUser = () => {
  const [sessions, setSessions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [filter, setFilter] = useState("accepted"); // Changed default filter
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/sessions/user/sessions?status=${filter}`, // Updated endpoint
        {
          headers: {
  ...
  const handleLeaveReview = async (sessionId) => {
    try {
      // Here you would typically open a modal to collect rating and comment
  ...
  const filteredSessions = sessions.filter((session) => {
    const mentorName = session.mentor
      ? `${session.mentor.firstName} ${session.mentor.lastName}`.toLowerCase()
      : "";
            ...
            <div key={session._id} className="bg-white rounded-lg shadow p-6">
              ...
              <div className="mt-6 flex justify-between items-center">
                ...
                {session.status === "completed" && !session.hasReview && (
                  <button
                    onClick={() => handleLeaveReview(session._id)}
